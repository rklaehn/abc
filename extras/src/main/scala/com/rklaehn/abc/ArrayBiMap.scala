...
final class ArrayBiMap[@sp(ILD) K, @sp(ILD) V] private[abc] (
  ...
  def exceptKeys(keys: ArraySet[K])(implicit kOrder: Order[K], vOrder: Order[V], vClassTag: ClassTag[V]): ArrayBiMap[K, V] = {
...
object ArrayBiMap {
  ...
  def singleton[@sp(ILD) K: Order, @sp(ILD) V: Order](k: K, v: V): ArrayBiMap[K, V] =
    new ArrayBiMap[K, V](
      ArrayMap.singleton[K, V](k, v),
      ArrayMap.singleton[V, K](v, k))

  def apply[@sp(ILD) K: Order: ClassTag, @sp(ILD) V: Order: ClassTag](kvs: (K, V)*): ArrayBiMap[K, V] = {
    new ArrayBiMap[K, V](
      ArrayMap(kvs: _*),
